...
class CommercialSalesReportController extends Controller
{
    public function __invoke(Request $request, Tenant $tenant, PlatformAccessPolicy $policy): JsonResponse
    {
        ...
        return response()->json([
            'summary' => ['transactions' => (int) $summary->transactions, 'net' => round((float) $summary->net, 2), 'tax' => round((float) $summary->tax, 2), 'total' => round((float) $summary->total, 2), 'cost' => round($cost, 2), 'margin' => round((float) $summary->net - $cost, 2)],
            'data' => collect($rows->items())->map(fn (InventorySale $sale) => ['id' => $sale->id, 'date' => $sale->sale_date?->format('Y-m-d'), 'source_type' => $sale->source_type, 'source_number' => $sale->source_number, 'document_type' => $sale->fiscal_document_type, 'operation_kind' => $sale->operation_kind, 'customer_name' => data_get($sale->metadata, 'customer_name'), 'payment_status' => data_get($sale->metadata, 'payment_status', 'paid'), 'net' => (float) $sale->net_amount * $sale->reporting_sign, 'tax' => (float) $sale->tax_amount * $sale->reporting_sign, 'total' => (float) $sale->total_amount * $sale->reporting_sign])->values(),
